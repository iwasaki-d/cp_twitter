  ...
  describe '#create' do
    context '正常系' do
      it 'ユーザーとイイねするツィートが揃えれば保存出来ること' do
        like = build(:like, user: User.find_by(id: 5), tweet: User.find_by(id: 1).tweets.last)
    ...
    context '異常系' do
      it '空ユーザーは登録できないこと' do
        like = build(:like)
        like.user_id = ''
        like.tweet = User.find_by(id: 1).tweets.last
  ...
  describe '#destroy' do
    context '正常系' do
      it 'イイねを削除してもイイね先のツィートは削除されないこと' do
        like = build(:like, user: User.find_by(id: 5), tweet: User.find_by(id: 1).tweets.last)
        liked_tweet_id = User.find_by(id: 1).tweets.last.id
        expect(like.save).to be_truthy

        like.destroy

        expect(Tweet.find_by(id: liked_tweet_id)).to be_present
      end
    end
  end
end
